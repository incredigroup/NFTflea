...
contract Marketplace is Ownable, Pausable, FeeManager, IMarketplace {
     ...
     */
    constructor(address _acceptedToken, address _artToken) Ownable() {
        require(
        ...
        require(
            _artToken.isContract(),
            "The accepted token address must be a deployed contract"
        );
        iArtToken = IArtToken(_artToken);
    }
        ...
        public whenNotPaused
    {
        ...
        // Send to creator royallty
        uint256 royalltyShareAmount = 0;
        if (_royalty > 0) {
            royalltyShareAmount= _priceInWei.mul(_royalty).div(1e6);
            
            acceptedToken.transferFrom(
                msg.sender, //buyer
                iArtToken.getTokenCreator(_assetId),
                royalltyShareAmount
            );
        }
    ...
    */
        ...
        public whenNotPaused
    {
        ...
        if(_royalty > 0) {
            royalltyShareAmount = bid.price.mul(_royalty).div(1e6);
